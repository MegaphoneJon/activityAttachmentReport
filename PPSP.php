 ...
 *
 */
class CRM_Report_Form_Activity_PPSP extends CRM_Report_Form {
  ...
  function __construct() {
    ...
    //{* Begin PPSP customization *}
    //Remove built-in activities (id <= 45);
    $this->activityTypes = array_flip($this->activityTypes);
    $this->activityTypes = array_filter($this->activityTypes, function ($x) { return $x > 45; });
    $this->activityTypes = array_flip($this->activityTypes);
    //{* End PPSP customization *}
    ...
    $this->_columns = array(
      ...
      //PPSP Customization - show attachments
      'civicrm_entity_file' =>
      array(
        ...
        array(
          'file_id' =>
          array('title' => ts('Attachments'),
            'type' => CRM_Utils_Type::T_INT,
          ),
        ),
      ),
      //End PPSP Customization - show attachments
    );
    ...
    parent::__construct();
    //{* Begin PPSP customization *}
		//remove more unneeded filters (Custom fields that don't need filtering)
		unset($this->_columns['civicrm_value_peer_educators_volunteers_interns_19']['filters']);
		unset($this->_columns['civicrm_value_gender_17']['filters']);
		unset($this->_columns['civicrm_value_race_ethnicity_16']['filters']);
		unset($this->_columns['civicrm_value_demographics_1']['filters']);
		unset($this->_columns['civicrm_value_evaluation_20']['filters']);
		unset($this->_columns['civicrm_value_encounter_info_4']['filters']['custom_59']);
		unset($this->_columns['civicrm_value_encounter_info_4']['filters']['custom_40']);
		unset($this->_columns['civicrm_value_educational_presentation_3']['filters']['custom_16']);
    //{* End PPSP customization *}
//		CRM_Core_Error::debug('this', $this->_columns);
//		CRM_Core_Error::debug('this', $this->_columns['civicrm_value_encounter_info_4']['filters']);
//		die;
...
//PPSP Customization - show attachments
// If attachment IDs are found, link to the attachments
//CRM_Core_Error::debug('row', $row);
      if (array_key_exists('civicrm_entity_file_file_id', $row) &&
          $row['civicrm_entity_file_file_id']) {
        $link = array();
    ...
    // custom code to alter rows
    //{* PPSP Customization Begins *}
    //create and initialize the totals row
    $rowCount = count($rows);
		$totalsRow = $rows[0];
		foreach($totalsRow as $key => $value) {
			$totalsRow[$key] = '';
		}
		...
		foreach($rows as $row) {
			foreach($row as $key => $column) {
				if(is_numeric($column)) {
					$totalsRow[$key] += $column;
				}
			}
		}

    //The activity type is always present.
    //So it's safe to put the "Totals:" label in there.
    $totalsRow['civicrm_activity_activity_type_id'] = 'Total Rows: ' . $rowCount;
    //put the totals row on the report
		$rows[] = $totalsRow;          
  }

	//include a custom countStat to subtract one from rows listed (for the totals row)
